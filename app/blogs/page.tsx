...
const defaultPageData = {
  blogPosts: [
    ...
    },
    {
      ...
      date: "2025-04-10",
      readTime: "5 min",
      image: "https://images.unsplash.com/photo-1521791136064-7986c2920216?auto=format&fit=crop&w=1469&q=80",
      likes: 89,
      type: "blog"
    }
  ],
...
export default function MediaGallery() {
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
     const { data, error } = await fetchPageData("blog");
     window.scrollTo({top:0, behavior:'smooth'});
      if (data && !error) setPageData(data as any);
      setLoading(false);
    };
    loadData();
  }, []);

  const { blogPosts, youtubeVideos } = pageData;
          ...
          <div className="flex flex-wrap justify-center mb-8 border-b border-gray-200">
            {['all', 'blogs', 'videos'].map((tab) => (
              <button
                ...
                className={`px-6 py-3 font-medium text-sm rounded-t-lg ${activeTab === tab
              ...
              <nav className="inline-flex rounded-md shadow">
                ...
                {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                  <button
                    ...
                    className={`px-4 py-2 border-t border-b border-gray-300 text-sm font-medium ${currentPage === page
